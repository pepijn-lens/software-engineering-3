...
class HillClimbSearch:
    ...
    def run_search(
        ...
        for i in tqdm(range(iterations), desc="Hill climbing"):
            # Generate neighbors
            neighbors = []
            ...
            n_workers = min(neighbors_per_iter, np.floor(cpu_count()*0.8))

            # Use multiprocessing to evaluate neighbors in parallel
